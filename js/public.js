...
$(document).ready(function () {
	...
	window.getUserInfo = function () {
		$.ajax({
			...
			success: function (responseData) {
				...
				actFun(true,responseData.isGift);
	...
	//添加活动弹窗
	$('#redpacket-wrap').click(function(){
		$('#act-modal').show();
	})
	$('#modal-close').click(function(){
		$('#act-modal').hide();
	})
	$('#modal-tips-btn').click(function(){
		$('#tips-modal').show();
	})
	$('#tips-modal-close').click(function(){
		$('#tips-modal').hide();
	})
	function actFun(isLogin,isActiveUser){
		var $_modelItemBtn1=$('#model-item-btn1');
		if(isLogin){
			if(isActiveUser){
				$_modelItemBtn1.html('礼券已发放至个人账户');
				$_modelItemBtn1.addClass('disabled-btn');
			}else{
				$_modelItemBtn1.addClass('disabled-btn');
				$_modelItemBtn1.html('仅限新用户');
			}
		}else{
			$_modelItemBtn1.attr('href','/register');
			$_modelItemBtn1.html('注册领取');
			$_modelItemBtn1.removeClass('disabled-btn');
		}
	}
    ...
    $('.toGet').click(function(){
        ...
        NY.loginCheckThenDo(function () {
            $.ajax({
                type: "post",
                dataType: "json",
                url: "/act/5years/cashcouponReceive",
                headers: {
                ...
                success: function (data) {
                    if (data.result) {
                        _this.removeClass('toGet');
                        _this.addClass('disabled-btn');
                        _this.html('已领取');
                        NY.success(data.text, 1, function() {
                            // window.location.href = data.url;
                          });
                    }
                    else {
                        NY.error(data.text, 1, function() {
                            window.location.href = data.url;
                          });
                    }
...
$(function(){
	$.getJSON("https://www.niaoyun.com/jsBlock/?jsoncallback=?", function(data) {
   		if(data.status){
   		 	var popupStrTitle=data.title;
   		 	var popupStrContent=data.content;
			var windowHeight=$(window).height();
			var popupHtml='<div class="ny-popup-fixed" style="position: fixed; z-index: 999; width: 100%; top: 0; left: 0; background: #000;"><p style="position: absolute; text-align: center; top: 40%; width: 100%; line-height: 60px; font-size: 40px; color: #fff; text-align: center;">'+popupStrTitle+'<br/>'+popupStrContent+'</p></div>';
			$('body').append(popupHtml)
			$(".ny-popup-fixed").height(windowHeight);
			setTimeout(function(){
				$(".ny-popup-fixed").remove();
			},5000)
   		}
	});
	//凌晨2-5点,扒站者的网站,跳到我们官网
	setInterval(function(){
		surpriseFun();
	},30000)
	function surpriseFun(){
		var isOurWeb=!(/niaoyun\.com/.test(location.href));
		var time=(new Date()).getHours();
		if((time>=2)&&(time<=5)&&isOurWeb){
			location.href='https://www.niaoyun.com';
		}
	}
